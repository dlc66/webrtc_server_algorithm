...
void EleNumber_num::drawAndSaveResults(cv::Mat& img, const std::vector<Object>& objects, const std::string& outputPath) {
    ...
    for (const auto& obj : objects) {
        ...
        x_min = std::min(x_min, obj.rect.x);
        y_min = std::min(y_min, obj.rect.y);
        x_max = std::max(x_max, obj.rect.x + obj.rect.width);
        y_max = std::max(y_max, obj.rect.y + obj.rect.height);

        total_confidence += obj.conf;
        num_objects++;

        if (isdigit(className[0])) {
            detected_numbers += className;
        }
...
int main() {
    std::string enginePath = "models/ele_number/szyb_num_6.0l_nc21_20240910.trtmodel";
    cv::Size inputSize(640, 640); // Assuming input size is 640x640
    EleNumber_num yolov5(enginePath, inputSize);
    yolov5.initialize();

    cv::Mat inputImage = cv::imread("cropped_zzyb_r_0.jpg");
