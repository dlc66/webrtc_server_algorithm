    ...
    if (!pModule) {
      ...
      for (const auto subalgorithm : *algorithm) {
        ...
        if(subalgorithm.name=="mean_filter")
        {
            ...
            input = PyObject_CallObject(mean_filter, pArgs); // 调用函数
            Py_DECREF(pArgs);
            Py_DECREF(mean_filter);
            if (!input) {
              PyErr_Print();
        ...
        if(subalgorithm.name=="adjust_contrast_and_lightness")
        {
            ...
            PyObject* pArgs = PyTuple_Pack(3, input,width,height); // 传递参数
            input = PyObject_CallObject(adjust_contrast_and_lightness, pArgs); // 调用函数
            Py_DECREF(pArgs);
            Py_DECREF(adjust_contrast_and_lightness);
            if (!input) {
              PyErr_Print();
...
int main(int argc, char* argv[]) {
  ...
  std::thread flv_2_rtp_thread(
      [webrtc_session_factory,&algorithm,rtsp]() { H2642Rtp(rtsp, webrtc_session_factory,&algorithm); }
      );
