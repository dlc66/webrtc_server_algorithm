...
class YbZsdClassier {
public:
    YbZsdClassier(const std::string& engine_file_path);
    ~YbZsdClassier();
...
private:
    ...
    const std::vector<std::string> labels = {"red","red_dark","green","green_dark","white","white_dark", "yellow","yellow_dark","blue","blue_dark","ptyb_off","ptyb_on","ptyb_nolink","small_dark"};
    cv::Mat preprocess(const cv::Mat& image);
};

YbZsdClassier::YbZsdClassier(const std::string& engine_file_path): engine(nullptr), runtime(nullptr), context(nullptr), stream(0), input_buffer(nullptr), output_buffer(nullptr) {
 // 加载 TensorRT 引擎
    ...
    output_size = 14;  // 因为有两个类别 'fenzha' 和 'hezha'
    // 分配 GPU 缓存
...
std::string YbZsdClassier::infer(const cv::Mat& input_image) {
...
cv::Mat YbZsdClassier::preprocess(const cv::Mat& image) {
