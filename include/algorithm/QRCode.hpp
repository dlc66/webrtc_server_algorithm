...
public:
    QRCodeClassifier(const std::string& engine_file_path);
    ~QRCodeClassifier();
...
QRCodeClassifier::QRCodeClassifier(const std::string& engine_file_path)
...
cv::Mat QRCodeClassifier::preprocess(const cv::Mat& image) {
...
std::pair<std::string, float> QRCodeClassifier::infer(const cv::Mat& input_image) {
