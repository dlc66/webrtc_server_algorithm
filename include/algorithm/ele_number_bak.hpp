...
class ElenumberDetect {
public:
    ElenumberDetect(const std::string& enginePath, cv::Size inputSize);
    std::vector<std::string> classNames_Num = {
...
ElenumberDetect::ElenumberDetect(const std::string& enginePath, cv::Size inputSize)
...
ElenumberDetect::~ElenumberDetect() {
    delete[] mBlob;
    delete[] mProb;
...
void ElenumberDetect::initialize() {//分配资源
    //cudaSetDevice(0);
...
void ElenumberDetect::preprocess(cv::Mat& src, cv::Mat& dst) {
...
void ElenumberDetect::blobFromImage(cv::Mat& img) {//cv::Mat转为mBlob
    preprocess(img, img);
...
int ElenumberDetect::doInference() {
...
std::pair<int, float> ElenumberDetect::argmax(std::vector<float>& vSingleProbs) {
...
void ElenumberDetect::generate_proposals(std::vector<Object>& objects, float confThresh) {
    int nc = 21;  //类别数量
    for (int i = 0; i < 25200; i++) {
        ...
        if (conf > confThresh) {
            ...
            obj.conf = conf;
              ...
              // 输出边界框的位置、实际坐标和类别信息
            std::cout << "Class: " << classNames_Num[obj.label] << ", "
...
//快速排序
void ElenumberDetect::qsort_descent_inplace(std::vector<Object>& objects, int left, int right) {
    int i = left;
    int j = right;
...
void ElenumberDetect::qsort_descent_inplace(std::vector<Object>& objects) {
    if (objects.empty())
        return;
...
float ElenumberDetect::intersection_area(const Object& a, const Object& b) {
    cv::Rect intersection = a.rect & b.rect;
    return intersection.area();
}//置信度交集IOU

void ElenumberDetect::nms_sorted_bboxes(const std::vector<Object>& vObjects, std::vector<int>& picked, float nms_threshold) {
    picked.clear();
...
std::vector<Object> ElenumberDetect::decodeOutputs(std::vector<Object>& objects) {
...
//输出结果的显示
void ElenumberDetect::drawAndSaveResults(cv::Mat& img, const std::vector<Object>& objects) {
    ...
    for (const auto& obj : objects) {
        // 获取类别名称
        std::string className = classNames_Num[obj.label];
...
// int main() {
//     std::string enginePath = "ele_number/";
//     cv::Size inputSize(640, 640); // Assuming input size is 640x640
//     ElenumberDetect Eledetect(enginePath, inputSize);
//     Eledetect.initialize();

//     cv::Mat inputImage = cv::imread("test/chongqing_bolin_20240408_zUux_b512858a-8be3-491a-9aff-3004ec1bf887.jpg");
//     Eledetect.blobFromImage(inputImage);
//     Eledetect.doInference();

//     std::vector<Object> objects;
//     std::vector<Object> results = Eledetect.decodeOutputs(objects);

//     // Draw and save the detection results
//     //Eledetect.drawAndSaveResults(inputImage, results, "output.jpg");
