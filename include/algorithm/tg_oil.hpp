...
class TgOilClassifier {
public:
    TgOilClassifier(const std::string& engine_file_path);
    ~TgOilClassifier();
...
TgOilClassifier::TgOilClassifier(const std::string& engine_file_path)
...
TgOilClassifier::~TgOilClassifier() {//释放
...
cv::Mat TgOilClassifier::preprocess(const cv::Mat& image) {//图像预处理
...
std::pair<std::string, float> TgOilClassifier::infer(const cv::Mat& input_image) {
