...
class GearClassifier {
public:
    GearClassifier(const std::string& engine_file_path);
    ~GearClassifier();
...
GearClassifier::GearClassifier(const std::string& engine_file_path)
...
cv::Mat GearClassifier::preprocess(const cv::Mat& image) {
...
std::pair<std::string, float> GearClassifier::infer(const cv::Mat& input_image) {
