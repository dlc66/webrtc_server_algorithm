...
public:
    ...
    uint32_t retrieve(char *buf, uint32_t length)
    {
        ...
        // Ensure requested length does not exceed available data
        ...
        if (length <= readable)
        {
            ...
            else
            {
                ...
                uint32_t first_part = _bufsize - r;
                std::copy(_buf + r, _buf + _bufsize, buf);
                std::copy(_buf, _buf + (length - first_part), buf + first_part);
                r = (r + length) % _bufsize; // Wrap around
            }
            set_rindex(r);
            set_buf_full(0);
            len = length;
        ...
        {
            // Handle cases where length requested is larger than readable size
            len = retrieve_all(buf);
        }
